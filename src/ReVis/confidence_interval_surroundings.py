...
def plot_confidence_intervals(before_filepath:str, after_filepath:str, num_sig_transcripts:int, num_all_transcripts:int, general_legend_names = True, all_before_filepath:str = "", all_after_filepath:str = "", all_transcripts:int = 0, filename = "cumulative_repeat_presence_around_transcripts_95_perc_CI", modelstats_filename = "pol_reg_sum.txt", plot_fourier_transform_only = False, legend = True, plot_white_bg = False):
    """
    ...
    for rep_class in rep_classes:
        ...
        if not plot_fourier_transform_only:
            ...
            before_model = sm.WLS(before_fourier_denoise, p_before, weights=np.full_like(before_dict[rep_class], num_sig_transcripts)).fit()
            all_before_model = sm.WLS(all_before_fourier_denoise, p_before, weights=np.full_like(all_before_dict[rep_class], num_all_transcripts)).fit()
            after_model = sm.WLS(after_fourier_denoise, p_after, weights=np.full_like(after_dict[rep_class], num_sig_transcripts)).fit()
            all_after_model = sm.WLS(all_after_fourier_denoise, p_after, weights=np.full_like(all_after_dict[rep_class], num_all_transcripts)).fit()
        ...
        #######  plot FAST FOURIER TRANSFORMATION
        elif plot_fourier_transform_only:

            def plot_ft_power_spectral_density(in_list):
                """
            ...
            psd_max = 50
            before_model_psd_plot_list, before_model = make_fft_of_rep_prop(before_dict[rep_class], psd_max = psd_max)
            after_model_psd_plot_list, after_model = make_fft_of_rep_prop(after_dict[rep_class], psd_max = psd_max)
            all_before_model_psd_plot_list, all_before_model = make_fft_of_rep_prop(all_before_dict[rep_class], psd_max = psd_max)
            all_after_model_psd_plot_list, all_after_model = make_fft_of_rep_prop(all_after_dict[rep_class], psd_max = psd_max)
            ...
            raw_transparency = 0.5
            ax.plot(x_before, before_model, label = f"{rep_label} fourier transform\nforeground", color = colors[rep_class], linewidth = 3, linestyle = (0, (5, 2)))
            ax.plot(x_before, before_dict[rep_class], color = colors[rep_class], linewidth=2, alpha = raw_transparency)
            ax.plot(x_after, after_model, color = colors[rep_class], linewidth=3, linestyle = (0, (5, 2)))
            ...
            if all_before_dict !={} and all_after_dict !={} and all_transcripts!=0:
                ...
                ax.plot(x_before, all_before_model, color = colors[rep_class],label = f"{rep_label} fourier transform \nbackground", linewidth=1, linestyle = (0, (5, 2)))
                ax.plot(x_before, all_before_dict[rep_class], color = colors[rep_class], linestyle = (0, (1, 10)), linewidth = 2, alpha = raw_transparency)                    
                ax.plot(x_after, all_after_model, color = colors[rep_class], linewidth=1, linestyle = (0, (5, 2)))
            ...
            filename_fft = filename.replace("_95_perc_CI", "_fft")
            filename_out = f"{filename_fft}_fft_{rep_class}.png"
